...
type LooseSelectBuilder = {
  eq(column: string, value: string): LooseSelectBuilder;
...
type LooseDbClient = {
  from(table: string): {
    insert(values: Record<string, unknown>): { select(columns: string): { single(): QueryResult<Record<string, unknown>> } };
...
function containsClientIdentity(value: unknown): boolean {
  if (!value || typeof value !== "object") return false;
  if (Array.isArray(value)) return value.some(containsClientIdentity);
  const record = value as Record<string, unknown>;
  if (["user_id", "userId", "client_user_id", "clientUserId"].some((key) => Object.prototype.hasOwnProperty.call(record, key))) return true;
  return Object.values(record).some(containsClientIdentity);
}
...
function bad(blockers: string[], status: number, gates: unknown) {
  return NextResponse.json({ ...disabled, blockers: Array.from(new Set(blockers)), gates }, { status });
}

function safeMetadata(value: unknown) {
  if (!isRecord(value)) return {};
  if (containsClientIdentity(value)) return { rejectedNestedIdentity: true };
  return value;
}

async function readback(db: LooseDbClient, table: string, input: { id: string; userId: string; namespace: string }) {
...
async function listContains(db: LooseDbClient, input: { id: string; userId: string; namespace: string }) {
  const result = await db
    .from("memory_items")
    .select("id")
    .eq("user_id", input.userId)
    ...
    .limit(25);
  return Boolean((result.data ?? []).some((row) => row.id === input.id) && !result.error);
}

async function runDirectAppendProof(input: {
  request: NextRequest;
  payload: LiveOnePayload;
  env: NodeJS.ProcessEnv;
  gates: unknown;
  ...
  const operatorMetadata = safeMetadata(input.payload.metadata);
  ...
  const item = await db
    ...
    .insert({
      ...
      confidence: 1,
      canon_status: "draft",
      source_summary: "first controlled live append proof",
      metadata: {
        reviewItemId,
        decisionId,
        proofOnly: true,
        ...
        appendOnly: true,
        contentFingerprint,
        rawContentRedactedFromProof: true,
        operatorMetadata,
      },
      created_at: now,
      updated_at: now,
  ...
  const source = await db
    ...
    .insert({
      ...
      excerpt: null,
      confidence: 1,
      metadata: { reviewItemId, decisionId, proofOnly: true, oneItemOnly: true, appendOnly: true },
      created_at: now,
    })
    .select("id")
    .single();
  ...
  const patch = await db
    ...
    .insert({
      ...
      namespace,
      memory_item_id: memoryItemId,
      patch_type: "ingest_append",
      reason: "first controlled live append proof",
      before_snapshot: null,
      after_snapshot: { memoryItemId, sourceId, reviewItemId, decisionId, appendOnly: true, contentRedacted: true },
      metadata: { proofOnly: true, oneItemOnly: true, idempotencyFingerprint },
      created_at: now,
    })
    .select("id")
    .single();
  ...
  const audit = await db
    ...
    .insert({
      user_id: userId,
      namespace,
      action: "first_live_append_proof_executed",
      table_name: "memory_items",
      record_id: memoryItemId,
      before_snapshot: null,
      after_snapshot: { memoryItemId, sourceId, patchId, reviewItemId, decisionId, appendOnly: true, contentRedacted: true },
      metadata: { proofOnly: true, oneItemOnly: true, idempotencyFingerprint },
      created_at: now,
    })
    .select("id")
  ...
  const proofBlockers = [
    readbackVerified ? null : "readback_failed",
    browserVisible ? null : "browser_visibility_failed",
    auditVerified ? null : "audit_verification_failed",
  ...
  return NextResponse.json(
    {
      ...
      warnings: ["stop_after_one_item", "do_not_proceed_to_retrieval"],
      receipt: {
        workflowId: `first-live-append-${receiptFingerprint}`,
        namespace,
        serverUserFingerprint: fingerprint(userId),
        reviewItemId,
        decisionId,
        memoryItemId,
        sourceId,
        ...
        readbackStatus: readbackVerified ? "verified" : "failed",
        browserVisibilityStatus: browserVisible ? "verified" : "failed",
        auditVerificationStatus: auditVerified ? "verified" : "failed",
        receiptFingerprint,
        redacted: true,
      },
      proof: {
        status: proofBlockers.length === 0 ? "ready_to_close" : "blocked",
        proofReportFingerprint: fingerprint({ receiptFingerprint, readbackVerified, browserVisible, auditVerified }),
        appendOnlyVerification: "verified",
        finalStopCondition: "Stop after one item. Do not batch. Do not proceed to retrieval.",
        redacted: true,
      },
      gates: input.gates,
      message: proofBlockers.length === 0 ? "First controlled live append proof executed. Stop now before retrieval." : "Append executed, but proof verification has blockers. Stop and investigate before any next step.",
    },
    { status: proofBlockers.length === 0 ? 200 : 500 },
  );
}
  ...
  return {
    ...
    async POST(request: NextRequest) {
      ...
      if (hasInjectedWorkflow) {
        ...
        if (body.typedConfirmation !== "APPEND MEMORY") blockers.push("typed_confirmation_mismatch");
        if (request.headers.get("x-pandora-internal-persistence-mode") !== "approved-review-executor") blockers.push("missing_internal_header");
        if (!runtime.config.approvedReviewPersistenceEnabled) blockers.push("approved_review_persistence_gate_disabled");
        if (!runtime.config.adminPersistenceConsoleEnabled) blockers.push("admin_persistence_gate_disabled");
        if (runtime.config.publicMemoryPersistenceEnabled || runtime.config.ingestProductionWriteEnabled || runtime.config.publicMemoryReadEnabled) blockers.push("unsafe_public_or_ingest_gate_enabled");
      ...
      return runDirectAppendProof({ request, payload: body, env, gates: runtime.gates, sessionResult: session });
    },
  };
}
