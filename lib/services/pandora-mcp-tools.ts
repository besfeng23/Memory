...
export async function getAdaptiveContextTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { const input=adaptiveContextInputSchema.parse(rawInput); const data=await buildAdaptiveChatGptContext(client,{user_id:principal.userId,namespace:input.namespace,query:input.query,current_task:input.current_task,max_items:input.max_items}); const audit=await auditPandoraMcpToolCall(client,{principal,tool:"mcp.get_adaptive_context",namespace:input.namespace}); return {...data,warnings:[...(data.warnings??[]),...warning(audit)]}; }
export async function analyzeMemoryCandidatesTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { const input=analyzeMemoryCandidatesInputSchema.parse(rawInput); const data=await createCandidatesFromSession(client,{user_id:principal.userId,namespace:input.namespace,source:input.source??"mcp_adaptive_analyze",source_ref:input.source_ref,text:input.text,mode:input.mode}); const audit=await auditPandoraMcpToolCall(client,{principal,tool:"mcp.analyze_memory_candidates",namespace:input.namespace}); return {...data,warnings:[...(data.warnings??[]),...warning(audit)]}; }
export async function semanticMemorySearchTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { const input=memoryContextInputSchema.parse(rawInput); const data=await getHybridMemoryContext(client,{user_id:principal.userId,namespace:input.namespace,query:input.query,current_task:input.current_task,max_items:input.max_items}); const audit=await auditPandoraMcpToolCall(client,{principal,tool:"mcp.semantic_memory_search",namespace:input.namespace}); return {...data,warnings:[...data.warnings,...warning(audit)]}; }
export async function createSessionDigestTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { const input=sessionDigestInputSchema.parse(rawInput); const data=await createSessionDigest(client,{user_id:principal.userId,namespace:input.namespace,source:input.source??"mcp_session",source_ref:input.source_ref,transcript_or_summary:input.transcript_or_summary,auto_capture:input.auto_capture,update_profiles:input.update_profiles,distill:input.distill}); const audit=await auditPandoraMcpToolCall(client,{principal,tool:"mcp.create_session_digest",namespace:input.namespace}); return {...data,warnings:[...(data.warnings??[]),...warning(audit)]}; }
export async function refreshAdaptiveProfilesTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { const input=z.object({namespace:namespaceSchema,profile_types:z.array(z.string()).optional(),subject_keys:z.array(z.string()).optional(),dry_run:z.boolean().optional()}).parse(rawInput); const result=await upsertProfileFromMemoryEvents(client,{user_id:principal.userId,namespace:input.namespace,profile_type:input.profile_types?.[0]??"operating_profile",subject_key:input.subject_keys?.[0]??"global",summary:"MCP adaptive profile refresh requested.",evidence_refs:[],dry_run:input.dry_run}); const audit=await auditPandoraMcpToolCall(client,{principal,tool:"mcp.refresh_adaptive_profiles",namespace:input.namespace}); return {result,warnings:warning(audit)}; }
export async function getOpenLoopsTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { const input=z.object({namespace:namespaceSchema}).parse(rawInput); const loops=await getOpenLoops(client,principal.userId,input.namespace); const audit=await auditPandoraMcpToolCall(client,{principal,tool:"mcp.get_open_loops",namespace:input.namespace}); return {open_loops:loops.data??[],warnings:[...(loops.error?[loops.error.message]:[]),...warning(audit)]}; }
export async function captureAdaptiveMemoryTool(client: MemoryBridgeDbClient, principal: Extract<PandoraMcpPrincipal, { ok: true }>, rawInput: unknown) { return analyzeMemoryCandidatesTool(client,principal,rawInput); }
