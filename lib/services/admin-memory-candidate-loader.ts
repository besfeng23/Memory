...
export async function listMemoryCandidatesForAdmin(client: MemoryBridgeDbClient, input: { userId: string; filters: CandidateListFilters }): Promise<{ candidates: MemoryCaptureCandidate[]; counts: Record<MemoryCandidateStatus, number> & { total: number }; warnings: string[] }> {
  ...
  return {
    candidates: (result.data ?? []).map(redact),
